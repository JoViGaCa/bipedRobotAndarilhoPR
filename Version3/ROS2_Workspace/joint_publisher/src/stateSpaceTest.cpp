...
class stateSpaceTest: public rclcpp::Node{
    public:
        stateSpaceTest():Node("stateSpaceTest"){
            ...
            this->sensor1 = this->create_subscription<sensor_msgs::msg::Imu>("imu_plugin1/out",50,std::bind(&stateSpaceTest::sensor1CallBack,this, _1));
            this->sensor2 = this->create_subscription<sensor_msgs::msg::Imu>("imu_plugin2/out",50,std::bind(&stateSpaceTest::sensor2CallBack,this, _1));

            timer_ = this->create_wall_timer(50ms, std::bind(&stateSpaceTest::timerCallBack, this));
        ...
        void applyControl(){
            // L(cx - zmp)
            aux2_obs[0] = matrixL[0]*(-stimate_state[2] + zmp_y);
            aux2_obs[1] = matrixL[1]*(-stimate_state[2] + zmp_y);
            aux2_obs[2] = matrixL[2]*(-stimate_state[2] + zmp_y);
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"Aux2: %.5f - %.5f - %.5f", aux2_obs[0], aux2_obs[1], aux2_obs[2]);

            //-kx(t)
            state_back = (stimate_state[0]*matrixK[0] + stimate_state[1]*matrixK[1] + stimate_state[2]*matrixK[2]);
            //state_back = (state_vector[0]*matrixK[0] + state_vector[1]*matrixK[1] + state_vector[2]*matrixK[2]);
            

            // B*u, and, u = (pref - state_back)
            aux1_obs[0] = matrixB[0]*(pref[count] - state_back);
            aux1_obs[1] = matrixB[1]*(pref[count] - state_back);
            aux1_obs[2] = matrixB[2]*(pref[count] - state_back);
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"Aux1: %.5f - %.5f - %.5f", aux1_obs[0], aux1_obs[1], aux1_obs[2]);

            // A*x

            aux3_obs[0] = matrixA[0][0]*stimate_state[0] + matrixA[0][1]*stimate_state[1] + matrixA[0][2]*stimate_state[2];
            aux3_obs[1] = matrixA[1][0]*stimate_state[0] + matrixA[1][1]*stimate_state[1] + matrixA[1][2]*stimate_state[2];
            aux3_obs[2] = matrixA[2][0]*stimate_state[0] + matrixA[2][1]*stimate_state[1] + matrixA[2][2]*stimate_state[2];
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"Aux3: %.5f - %.5f - %.5f", aux3_obs[0], aux3_obs[1], aux3_obs[2]);

            stimate_state[0] = aux1_obs[0] + aux2_obs[0] + aux3_obs[0];
            stimate_state[1] = aux1_obs[1] + aux2_obs[1] + aux3_obs[1];
            stimate_state[2] = aux1_obs[2] + aux2_obs[2] + aux3_obs[2];

            center_mass[0] = stimate_state[0];
...
int main(int argc, char **argv){
    ...
    rclcpp::spin(std::make_shared<stateSpaceTest>()); 
    rclcpp::shutdown();
    return 0;
}